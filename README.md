...
The GitLab `mirror` job is the source-side provenance gate: it pushes exactly the GitLab `gh-pages` commit to GitHub. The GitHub Pages workflow runs `npm run audit:provenance` in mirror mode to verify that the received push is the `gh-pages` workflow SHA; GitHub-hosted runners cannot resolve the self-managed GitLab hostname. Run the stronger direct equality check locally with `RELEASE_SHA=<full-commit-sha> npm run audit:provenance`.
