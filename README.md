...
- Report and principal-dialogue forms use the configured Function route (`PUBLIC_BRIEF_API_URL` for reports, or `PUBLIC_CONTACT_API_URL` for contact when set, falling back to the brief route). Set it to the Front Door-backed Function route before promoting the gated workflow; newsletter remains on the legacy/manual routing path until its CRM workflow is approved.
...
The GitLab `mirror` job pushes exactly the GitLab `gh-pages` commit to GitHub. The GitHub Pages workflow requires a detached Ed25519 assertion from the GitLab handoff before it builds; provision the protected inputs described in [`docs/release-provenance.md`](docs/release-provenance.md). The workflow passes `github.sha` only as the received commit to compare with that signed assertion; it never treats `GITHUB_SHA` or a branch name as provenance. For the direct remote check, use `RELEASE_SHA=<full-commit-sha> npm run audit:provenance`.
