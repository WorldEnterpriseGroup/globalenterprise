...
Cloudflare Browser Insights should remain disabled while Plausible is the approved analytics system; otherwise its injected script is blocked by the intentional CSP. If Browser Insights is enabled, disable it in the Cloudflare Web Analytics settings or add a zone Configuration Rule with `disable_rum: true` and an all-request expression; that path requires `Zone > Config Rules > Edit`. After the edge rule and telemetry choice are applied, run `LIVE_SITE_URL=https://globalenterprise.com npm run audit:security` and a browser console check.
